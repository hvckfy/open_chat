...
fyne_app_toml_value() {
  local key="$1"
  # `fyne package` rewrites this file itself (e.g. bumping Build= on
  # every run) and reformats it with indented keys under [Details] —
  # so the key may or may not be at column 0 depending on whether
  # anything has packaged since this repo was cloned. Tolerate leading
  # whitespace, and `|| true` the whole pipeline: under `set -o
  # pipefail`, grep finding no match exits 1, which would otherwise
  # abort this entire script the moment a key isn't found — that should
  # fall back to the caller's default, not kill the build.
  grep -E "^[[:space:]]*${key}[[:space:]]*=" cmd/app/FyneApp.toml 2>/dev/null | head -n1 | sed -E 's/^[^=]+=[[:space:]]*"?([^"]*)"?[[:space:]]*$/\1/' || true
}
