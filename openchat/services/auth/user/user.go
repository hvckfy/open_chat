...
func GetUser(username string) (User, bool, error) {
	...
	var u User
	query := `SELECT id, username, mail, phone, first_name, second_name, password_hash, auth_type FROM users WHERE username = $1`
	row := db.QueryRow(query, username)
	err = row.Scan(&u.App.UserId, &u.App.Username, &u.Personal.Mail, &u.Personal.Phone, &u.Data.FirstName, &u.Data.SecondName, &u.App.Password, &u.App.AuthType)
