...
# this repo builds (app/golang/scripts/build-client.sh derives the same
...
#
# Usage (from anywhere — path resolution below is self-locating):
#   ./cicd/scripts/build-nodes.sh                # build only
#   ./cicd/scripts/build-nodes.sh --push         # build and push both tags
#   TAG=v1.2.3 ./cicd/scripts/build-nodes.sh      # override the version tag
#   REGISTRY=myregistry.example.com/openchat ./cicd/scripts/build-nodes.sh
#
set -euo pipefail

# This script lives at <repo root>/cicd/scripts/ — the Go module it builds
# lives at <repo root>/app/golang/ (that's the Docker build *context*, so
# .dockerignore/COPY semantics inside Dockerfile.node|client see exactly
# the module and nothing else), and the Dockerfiles themselves live next
# to this script's parent, at <repo root>/cicd/docker/.
REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
APP_DIR="$REPO_ROOT/app/golang"
DOCKER_DIR="$REPO_ROOT/cicd/docker"
cd "$REPO_ROOT"

if [ ! -f "$APP_DIR/go.mod" ]; then
  echo "error: $APP_DIR doesn't look like the openchat Go module (no go.mod) — repo layout changed?" >&2
  exit 1
fi
...
# Same version-derivation scheme as app/golang/scripts/build-client.sh, so
# a binary build and the Docker images built from the same commit always
...
build_and_tag() {
  ...
  docker build -f "$dockerfile" -t "$REGISTRY/$name:$TAG" -t "$REGISTRY/$name:latest" "$APP_DIR"
}
...
# this image's tag.
build_and_tag "$DOCKER_DIR/Dockerfile.node" node
...
build_and_tag "$DOCKER_DIR/Dockerfile.client" client
